...
function configure(gyp, argv, callback) {
  ...
  function createPythonSymlink() {
    ...
    log.verbose(
      "python symlink",
      `creating symlink to "${python}" at "${symlinkDestination}"`
    );
    ...
    fs.unlink(symlinkDestination, function (err) {
      if (err && err.code !== "ENOENT") {
        log.verbose(
          "python symlink",
          "error when attempting to remove existing symlink"
        );
      ...
      fs.symlink(python, symlinkDestination, function (err) {
        if (err) {
          log.verbose(
            "python symlink",
            "error when attempting to create Python symlink"
          );
          log.verbose("python symlink", err.stack, "errno: " + err.errno);
        }
      });
  ...
  function createConfigFile(err, vsInfo) {
    ...
    createConfigGypi({ gyp, buildDir, nodeDir, vsInfo })
      .then((configPath) => {
  ...
  function runGyp(err) {
    ...
    if (process.platform === "os390") {
      ...
      if (zoslibIncPath) {
        zoslibIncPath = findAccessibleSync(logprefix, zoslibIncPath, [
          "zos-base.h",
        ]);
      ...
      if (zoslibIncPath !== undefined) {
        ...
        log.verbose(
          logprefix,
          "Found zoslib's zos-base.h in: %s",
          zoslibIncDir
        );
