...
const localAppData =
  process.env.LOCALAPPDATA || `${systemDrive}\\${username}\\AppData\\Local`;
  ...
  checkExecPath: function checkExecPath(execPath, errorCallback) {
    ...
    this.run(
      ...
      function (err, version) {
        ...
        if (!valid) {
          this.addLog(
            `- version is ${version} - should be ${this.semverRange}`
          );
          this.addLog("- THIS VERSION OF PYTHON IS NOT SUPPORTED");
          return errorCallback(
            new Error(`Found unsupported Python version ${version}`)
          );
        }
