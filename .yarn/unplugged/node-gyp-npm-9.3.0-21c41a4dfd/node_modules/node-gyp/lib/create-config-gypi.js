...
async function getBaseConfigGypi({ gyp, nodeDir }) {
  ...
  const useCustomHeaders =
    gyp.opts.nodedir || gyp.opts.disturl || gyp.opts["dist-url"];
  const shouldReadConfigGypi =
    useCustomHeaders && !gyp.opts["force-process-config"];
  if (shouldReadConfigGypi && nodeDir) {
    try {
      const baseConfigGypiPath = path.resolve(
        nodeDir,
