...
const execFileSync =
  childProcess.execFileSync || require("./process-exec-sync");
...
function runHello(hostProcess) {
  ...
  return execFileSync(hostProcess, ["-e", testCode], {
    cwd: __dirname,
  }).toString();
}
...
test("addon works with renamed host executable", function (t) {
  ...
  var notNodePath = path.join(
    os.tmpdir(),
    "notnode" + path.extname(process.execPath)
  );
