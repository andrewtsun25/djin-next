...
function allVsVersions(t, finder) {
  finder.findVisualStudio2017OrNewer = (cb) => {
    const data0 = JSON.parse(
      fs.readFileSync(path.join(__dirname, "fixtures", "VS_2017_Unusable.txt"))
    );
    ...
    const data3 = JSON.parse(
      fs.readFileSync(path.join(__dirname, "fixtures", "VS_2017_Express.txt"))
    );
    const data4 = JSON.parse(
      fs.readFileSync(path.join(__dirname, "fixtures", "VS_2019_Preview.txt"))
    );
    ...
    const data = JSON.stringify(
      data0.concat(data1, data2, data3, data4, data5, data6)
    );
    finder.parseData(null, data, "", cb);
  };
  finder.regSearchKeys = (keys, value, addOpts, cb) => {
